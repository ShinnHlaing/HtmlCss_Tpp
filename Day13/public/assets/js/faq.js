    ...
    .then((data) => {
      ...
      for (let index = 3; index < boxData.length - 2; index++) {
        cardtwo.innerHTML += `<div class="flex flex-col justify-center gap-3">
                  <div class="faqTitle2 flex justify-between items-center border-b" onclick="openBox(this)">
                    <div class="faq">${boxData[index].name}</div>
                    <div class="plus">+</div>
                  </div>
                  <div class="faqText2 hidden">
                    <p>${boxData[index].instructions[0]}</p>
                  </div>
                </div>`;
      }
...
function openBox() {
  let faq = document.getElementsByClassName("faqTitle2");
  let text = document.getElementsByClassName("faqText2");
  let plus = document.getElementsByClassName("plus");
}
